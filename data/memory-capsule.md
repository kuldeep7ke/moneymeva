...
- **Quota protection (v7.1.1.93+)**: apps fetch `https://moneymevaonline.pages.dev/api/announcements?type=broadcast|banner` — a Cloudflare Pages Function (`functions/api/announcements.js`, plain JS so Next tsc ignores it) that fetches jsonbin as origin and edge-caches via Cache API + `Cache-Control`. Cache window = `TTL_MINUTES` (currently **180 = 3 hours**, raised from 10). jsonbin volume is time-bound, not user-bound: ~8×/day/bin ≈ 16/day combined ≈ 480/month worst case (per Cloudflare POP) — far under the 10k free cap, with ample headroom to lower TTL for faster edits if ever needed. Edits propagate in up to ~3 hours
...
- **Broadcast pill** (`BroadcastBanner.tsx`): floating pill top-center over content — wrapper `fixed left-1/2 md:left-[calc(50%+8rem)] -translate-x-1/2 z-[9998]`, stacked via inline `style={{top: `${8+i*44}px`}}`; **pill has NO positioning of its own**, only swipe-to-dismiss `transform: translateX(dragX)` (v7.3.0.25 — the previous double `-50%` shifted it a full width off-center: v4 translate utilities use the independent `translate` property, so class + inline transform ADD); solid color-coded bg (info=blue-600, warning=amber-500, success=green-600, error=red-600); optional `link` wraps pill in anchor + ExternalLink icon; dismiss by X or swipe-left ≥70px; per-ID dismissal in localStorage `mm_dismissed_broadcasts`; `pinned: true` = no dismiss; array format = multiple stacked pills; fetched list cached at module level (`broadcastCache`) so navigation never refetches
...
- **Broadcast pill misplacement cause** (v7.3.0.21 regression, caught in v7.3.0.25): swipe-to-dismiss added an inline `transform: translateX(calc(-50% + dragX))` on a pill already carrying Tailwind `-translate-x-1/2`. In **Tailwind v4** the utility sets the CSS `translate` property (independent), so class −50% AND inline −50% both apply → pill sat a full pill-width left of center, visually overlapping the dashboard "AT A GLANCE" header. Pre-v7.3.0.21 there was no inline transform, so only the class applied and it looked correct.
- **Fix**: positioning moved to the `fixed` wrapper only — `left-1/2 md:left-[calc(50%+8rem)] -translate-x-1/2 z-[9998]`, `top: 8+i*44` stack — while the pill keeps just its swipe transform. Desktop pills now center over the **content area** (offset = half the `w-64` sidebar) instead of the whole viewport. Rule recorded: one element = one centering mechanism; if you need motion on the same element, do the centering on a wrapper.
- Verified: `npx tsc --noEmit` clean; bundle check confirmed the new class/transform split is in the served `out/`. Both fixes deployed through all 4 pipelines (Cloudflare, GitHub Pages, APK, Docker) on v7.3.0.24 and v7.3.0.25.
...
- **Diagnostics**: Sync Health (masked URL, sync account, connected/failed/untested status, last sync event via `getLastSyncEvent`) + **Test Connection** button with toast feedback; Local Database stats across all 11 tables (friendly labels: partners→Party, partnership_entries→Partnership Entry, mutation_log→Audit Log); storage usage; localStorage key/value inspector.
- **Danger Zone**: notice box (not reversible / export first / disable cloud sync on other devices) + compact `max-w-sm` button row — Start Fresh (Clear Remote + Push Local), Clear Remote Only, Clear Local Only, Clear ALL Data (Local + Remote). All pull/push/start-fresh/clear-remote actions blocked when not connected.
- **Docs policy**: the developer page is for the author only — documented ONLY in `From-Scratch.md` and the memory capsule. References removed from `README.md`, `docs/File-Map.md`, `docs/Changelog.md`, `docs/BROADCAST-GUIDE.md`. Party redesign also shipped this version: 3 group constants (vendor/customer/contact) → 7 groups (personal/services/financial/business/government/agriculture/office) with `src/lib/parties.ts` as the single source, auto-migration in `initDB` (`mm_partner_groups_v2` flag), type remap for old records (vendor→business, customer→business, contact→personal).
