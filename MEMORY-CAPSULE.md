...
- **Bin**: single combined bin `6aafc4baac6210605ae254df` — record shape `{ broadcasts: [...], banner: {...} }`
- **Quota protection**: apps fetch `https://moneymevaonline.pages.dev/api/announcements` — a Cloudflare Pages Function (`functions/api/announcements.js`, plain JS so Next tsc ignores it) that fetches jsonbin as origin and edge-caches via Cache API + `Cache-Control`. Cache window = `TTL_MINUTES` (currently **180 = 3 hours**). jsonbin volume is time-bound, not user-bound (~8×/day ≈ 240/month worst case) — far under the 10k free cap.
- **Fallback chain**: proxy fail → direct jsonbin `?t=${Date.now()}` + `cache: 'no-store'` (Bin ID stays in env.ts for this) — announcements never go dark
- **Wiring**: Bin ID + URL stored as XOR+base64 obfuscated constants in `src/lib/env.ts` (`ANNOUNCEMENTS_BIN_ID`/`JSONBIN_BASE`/`ANNOUNCEMENTS_API`, runtime `_d()` decoder with key `'moneymeva'`) — invisible to bundle extraction; verified zero plain-text occurrences in `out/`. Function has its own hardcoded `FALLBACK_BIN_ID` (overridable via the Pages env var `ANNOUNCEMENTS_BIN_ID`)
...
- **Banner modal** (`BannerModal.tsx`): full-screen overlay `fixed inset-0 z-[10000] flex items-center justify-center bg-black/60 backdrop-blur-sm`, centered card, optional image (max-h-64) + href; **never paints while deciding** — during fetch the modal returns `null`, so an absent/expired/bannerless bin shows NO loading flash on hard reload; a confirmed in-window banner mounts directly, with a skeleton card until content is ready; countdown (7s) starts only after full display (waits for image `onLoad`/`onError` with cached-image `complete` check); shows **once per app load** via module flag `bannerShownThisLoad`; NO localStorage persistence; scheduling via inclusive local-calendar-day windows through shared `isWithinPeriod(startDate?, endDate?)` in `utils.ts`
- Local `public/broadcast.json`/`public/banner.json` are dead fallbacks only
- Full editing workflow: `docs/BROADCAST-GUIDE.md`; Developer Zone → Remote Announcements tests BOTH proxy and jsonbin paths live (combined bin)
